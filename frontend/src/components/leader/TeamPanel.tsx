...
export function TeamPanel({ team, maps, onRefresh }: Props) {
  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[minmax(260px,320px)_1fr]">
      <ul className="bg-card border border-border rounded-2xl divide-y divide-border overflow-hidden shadow-sm">
        {assignableTeam.length === 0 ? (
          <li className="px-4 py-8 text-sm text-muted text-center">No team members found.</li>
        ) : (
        ...
        {!selectedMember ? (
          <div className="flex flex-col items-center justify-center h-full min-h-[280px] px-8 text-center">
            ...
            <p className="text-sm text-muted mt-1 max-w-xs">
              Tap a name to view their profile, assigned maps, and assign tasks
            </p>
          </div>
        ) : (
          <>
                  ...
                  return (
                    <li key={map.id} className="px-5 py-4 hover:bg-orange-50/30 transition-colors">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="text-xs font-medium text-muted shrink-0 px-2 py-1 rounded-md bg-slate-50">
